    ...
    def _map_sections(self, source_sections, target_sections):
        ...
        for source_section in source_sections:
            ...
            if not target_section:
                ...
                source_section.old_hash = target_section.old_hash
                source_newer = source_section.modified > target_section.modified

                source_changed = source_section.hash != source_section.old_hash
                target_changed = target_section.hash != target_section.old_hash

                # Target and source differ
                if target_section.hash != source_section.hash:

                    if source_changed:
                        source_to_target.append(source_section)
                    elif (source_section.hash == target_section.old_hash and source_newer):
                        # Source identical to old target, but file is newer
                        # This happens when you edit the target and then revert the edit on the (changed) source
                        source_section.old_hash = None # Hack (?)
                        source_to_target.append(source_section)
                    else: # Target has been changed
                        target_section.old_hash = target_section.hash # Hack to skip target rewrite
                        source_to_target.append(target_section) # Write section to target file
                        target_to_source.append(target_section) # Write section to source file

                else: # Target and source are identical
                    # source_section.old_hash = target_section.hash # Required?
                    source_to_target.append(source_section) # Write section to target file
